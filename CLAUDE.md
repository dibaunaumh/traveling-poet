...
- `Journal.Paragraphs` splits an entry's prose sections into paragraphs keyed by a hash of their visible text (the browser computes the same key from a rendered `<p>`), and `paragraph_subjects` puts each on the subject tree, tagged on every publish (`TopicTagging.tag_paragraphs_async/1`; a revision tags only the paragraphs it changed). `Journal.PageSubjects` derives a whole page's subjects from its tagged paragraphs, places, finds and excursion topic (weighted, never stored). It is the base of the reader's taste profile (card-49).
- `Reading` keeps how long the owner spends on each paragraph of their own journal (`paragraph_reads`, per key per day). The `ReadingTime` hook (a hidden element beside the entry on `/journal` only) measures and pushes `paragraph_reads`; `Reading.record/3` keeps only keys that are prose paragraphs of that entry, owned by that user, with `users.reading_signals` on (default on; turning it off in Settings calls `Reading.forget/1`). Never measure a public reader. Excluded from the change stream.
- `Affinity` is the reader's taste profile (cards 49 and 37): `{subject path, score}` pairs on the subject tree, computed on demand (never stored) from the owner's own signals (kept topics and tastes +4 without decay; markers ±3 on the paragraphs they mark; reactions ±2 over `PageSubjects`; paragraphs read through +1), each halving every 30 days. `subject_dismissals` is sticky. `Affinity.Layout` puts every topic at a fixed 2D point (a treemap sized by the tree, not by data) and `AffinityComponents.taste_map/1` draws a profile on it as green/red dots in Settings.
