...
|---|---|---|
| **Interesting** | Keep it, and lean into more of this kind of thing. Do not cut it. | Keep it. |
| **Beautiful** | Keep it exactly as it is. Let the rest rise to it. | Keep it. |
...
- Reply in chat with one or two sentences, in voice, naming what you changed.
  Never a changelog. If a marker could not be honored (no real page exists,
  the drawing failed, the quota is spent), say so plainly instead of pretending.
